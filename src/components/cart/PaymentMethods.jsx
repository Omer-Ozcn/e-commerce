...
import {
  fetchCards, createCard, updateCard, deleteCard, selectCard,
} from "../../store/thunks/cardThunks";

// demo: kart BIN'ine göre taksit seçenekleri (örnek/veri yoksa local hesap)
...
export default function PaymentMethods() {
  ...
  const selectedCard = useMemo(() => list.find((c) => c.id === selectedId) || null, [list, selectedId]);
  const installments = useMemo(() => computeInstallments(selectedCard?.card_no), [selectedCard]);
  ...
  const onSaveNew = async (payload) => {
    try {
      ...
      alert(e?.response?.data?.message || e.message || "Kart eklenemedi.");
    }
  };
  ...
  const onSaveEdit = async (payload) => {
    try {
      ...
      alert(e?.response?.data?.message || e.message || "Kart güncellenemedi.");
    }
  };
  ...
  const onDelete = async (id) => {
    ...
    try {
      ...
      alert(e?.response?.data?.message || e.message || "Kart silinemedi.");
    }
  };
            ...
            {installments.map((p) => {
              ...
              return (
                <label key={p.count} className="flex items-center justify-between p-3 border rounded mb-2 cursor-pointer">
                  ...
                  <span className="font-semibold">{per.toLocaleString("tr-TR",{style:"currency",currency:"TRY"})}</span>
                </label>
              );
            })}
          </div>
