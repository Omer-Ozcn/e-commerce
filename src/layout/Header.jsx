import {
  ChevronDown, ChevronUp, CircleUserRound, Heart, Mail, Menu, Phone, Search, ShoppingCart,
} from "lucide-react";
...
const keyFromCode = (code = "") =>
  trToAscii((code.split(":")[1] || "").toString())
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
const viewOf = (cat) => {
...
export default function Header() {
  ...
  const avatarSrc = (size = 32) =>
    customSrc ||
    `https://www.gravatar.com/avatar/${md5(email)}?s=${size}&d=identicon&r=g`;
  ...
  return (
    <header className="z-50 relative w-full font-[Montserrat] bg-white">
      <div
        className={`hidden md:flex justify-between items-center px-10 py-4 text-sm font-bold ${topbarBg} text-white`}
      >
      ...
      <div className="hidden md:flex justify-between items-baseline px-10 py-5 shadow-md">
        <div className="flex items-baseline gap-20">
          <Link className="text-2xl font-bold text-[#252B42] leading-[1.2]" to="/">
            Bandage
          </Link>
          ...
          <nav className="flex items-center gap-6 text-[#737373] text-sm font-bold">
            ...
            <div className="relative" onMouseEnter={openShop} onMouseLeave={scheduleClose}>
              <div className="flex items-center gap-1 cursor-pointer select-none">
                <Link to="/shop" className={location.pathname === "/shop" ? "font-normal" : ""}>
                  Shop
                </Link>
                {isShopOpen ? (
                  <ChevronUp
                ...
                >
                  <div className="flex flex-col w-[186px]">
                    <div className="flex items-start px-6 py-4 h-[56px]">
                      <span className="text-[#252B42] text-[14px] font-bold leading-6 tracking-[0.2px]">
                        Women
                      </span>
                    </div>
            ...
            <Link
              to="/contact"
              className={location.pathname === "/contact" ? "font-normal" : ""}
            >
              Contact
            </Link>
            <Link
              to="/pricing"
              className={location.pathname === "/pricing" ? "font-normal" : ""}
            >
              Pricing
            </Link>
          </nav>
        ...
        <div className="flex items-center gap-4 text-[#252B42]">
          <div className="flex gap-2 items-center">
            {isAuthed ? (
              ...
              <>
                <Link to="/maintenance" className="flex gap-1 items-center ">
                  <CircleUserRound />
                </Link>
                <Link to="/login" className="hidden md:flex text-[#23A6F0] font-bold">
                  Login
                </Link>
                <span className="hidden md:flex text-[#23A6F0] font-bold">/</span>
                <Link to="/register" className="hidden md:flex text-[#23A6F0] font-bold">
                  Register
                </Link>
              </>
            )}
      ...
      <div className="flex items-center justify-between px-6 py-5 md:hidden">
        <Link to="/" className="text-2xl font-bold text-[#252B42] leading-[1.2]">
          Bandage
        </Link>
      ...
      {isMobileOpen && (
        <nav
          id="mobile-nav"
          className="md:hidden flex flex-col items-center justify-center gap-6 py-10 text-[#737373] text-xl font-semibold"
        >
          <Link
            to="/"
            onClick={() => setIsMobileOpen(false)}
            className={location.pathname === "/" ? "text-[#252B42]" : ""}
          >
            Home
          </Link>
          <Link
            to="/shop"
            onClick={() => setIsMobileOpen(false)}
            className={location.pathname === "/shop" ? "text-[#252B42]" : ""}
          >
            Product
          </Link>
          <Link
            to="/about"
            onClick={() => setIsMobileOpen(false)}
            className={location.pathname === "/about" ? "text-[#252B42]" : ""}
          >
            About
          </Link>
          <Link
            to="/contact"
            onClick={() => setIsMobileOpen(false)}
            className={location.pathname === "/contact" ? "text-[#252B42]" : ""}
          >
            Contact
          </Link>
          <Link
            to="/pricing"
            onClick={() => setIsMobileOpen(false)}
            className={location.pathname === "/pricing" ? "text-[#252B42]" : ""}
          >
            Pricing
          </Link>
          {isAuthed && (
