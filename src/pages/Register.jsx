...
export default function Register() {
  ...
  useEffect(() => { dispatch(fetchRolesIfNeeded()); }, [dispatch]);
  ...
  const onSubmit = async (data) => {
    ...
    if (roleIdNum === storeRoleId) {
      if (!isValidTRPhone(data.store_phone)) {
        toast.error("Please enter a valid Türkiye phone number (05xx..., +90...)");
        return;
      }
      if (!isValidTRIBAN(data.bank_account)) {
        toast.error("Please enter a valid TR IBAN (TR + 24 digits)");
        return;
      }
    }
    ...
    if (roleIdNum === storeRoleId) {
      payload.store = {
        ...
        phone: normalizeTRPhone(data.store_phone),
        tax_no: (data.tax_no || "").toUpperCase(),
        bank_account: normalizeIBAN(data.bank_account),
      };
    }
    ...
    <main className="w-[90vw] md:w-[50vw] mx-auto my-20 flex flex-col gap-5 font-[Montserrat] text-[#252B42]">
      <div>
        ...
        <p className="text-center">
          Already a member?{" "}
          <Link to="/login" className="text-[#23A6F0]">Log in</Link>
        </p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 text-lg w-[90vw] md:w-[45vw] mx-auto">
        ...
        <div className="flex flex-col gap-2">
          ...
          <select
            ...
            className="border border-[#E6E6E6] p-1 rounded text-base disabled:bg-gray-100"
          >
            {!roles.length ? (
        ...
        {Number(selectedRole) === storeRoleId && (
          <>
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                {...register("store_phone", {
                  ...
                  validate: (v) => isValidTRPhone(v) || "Invalid TR phone number",
                })}
                className="border border-[#E6E6E6] p-1 rounded text-base"
              />
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                id="tax_no"
                placeholder="Format: T1234V123456"
                {...register("tax_no", {
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className="border border-[#E6E6E6] p-1 rounded text-base"
              />
              {errors.bank_account && <p className="text-red-500">{errors.bank_account.message}</p>}
            </div>
          </>
        )}
        ...
        <button
          ...
          className="bg-[#23A6F0] text-white font-bold py-2 rounded flex justify-center items-center w-full md:w-[15vw] mx-auto cursor-pointer hover:bg-[#2497da] disabled:opacity-70"
        >
